...
public class SlimeCore extends ItemBlock implements IHasModel
{
	...
	public SlimeCore(String name, Boolean inCreativeTab, @Nullable CreativeTabs tab, int size, Block slimeCore)
	{
	super(slimeCore);
        	 ...
        	 if(player.getHeldItem(hand).getTagCompound().hasKey("FLUID")){
        		...
        		if(player.getHeldItem(hand).getTagCompound().hasKey("FLUID")){
        			entitySpawn.setCoreFluid(player.getHeldItem(hand).getTagCompound().getCompoundTag("FLUID"));
        		 	...
        		 	NBTTagCompound emptyMaster = player.getHeldItem(hand).getTagCompound().getCompoundTag("FLUID").copy();
        		 	emptyMaster.setInteger("Amount", 0);
        		 	
        		 	entitySpawn.setMasterCoreFluid(emptyMaster);
        		...
        		player.inventory.clearMatchingItems(this, -1, 1, null);
         		...
         		if(player.getHeldItem(hand).getTagCompound().hasKey("FLUID")){
        		 	...
        		 	entityrainbowSpawn.setMasterCoreFluid(emptyMaster);
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn)
	{
		...
		if(!stack.hasTagCompound() ) {
			...
			tooltip.add("Infused Liquid: " + TextFormatting.WHITE +  "None");
		...
		if(stack.hasTagCompound()) {
			...
			if(stack.getTagCompound().hasKey("FLUID")){
					...
					tooltip.add("Infused Liquid: " + TextFormatting.YELLOW + "Yes");
